...
namespace FUTOMedical.Areas.Laboratorist.Controllers
{
    public class UserManagerController : Controller
    {
        private ApplicationDbContext db = new ApplicationDbContext();
        private ILaboratoristService _laboratoristService = new LaboratoristService();
        ...
        public UserManagerController(UserManagerService userService, ApplicationSignInManager signInManager, ApplicationUserManager userManager, ApplicationRoleManager roleManager, LaboratoristService laboratoristService)
        {
        ...
        public async Task<ActionResult> NewLaboratorist()
        {
        ...
        public async Task<ActionResult> NewLaboratorist(LaboratoristDto model, HttpPostedFileBase picture)
        {
            var ee = "";
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    succed = await _laboratoristService.NewLaboratorist(model);
                    if (succed == "true")
                    {
                        ...
                        //profile pic upload
                        var img = await db.Laboratorists.FirstOrDefaultAsync(x => x.UserId == user.Id);
                        img.Picture = model.Picture;

                        db.Entry(img).State = EntityState.Modified;
                        await db.SaveChangesAsync();

                        TempData["success"] = "New Laboratorist with username <i> " + model.Fullname + "</i> Added Successfully";
                        return RedirectToAction("NewLaboratorist");
                    }
                    else
                    {
            ...
            TempData["error"] = "Creation of new Laboratorist not successful" + ee;
            ViewBag.StateOfOrigin = new SelectList(db.States.OrderBy(x => x.StateName), "StateName", "StateName", model.StateOfOrigin);
            return View(model);
        }


        public async Task<ActionResult> EditLaboratorist(int? id)
        {
            if (id == null)
            {
            ...
            Laboratorists data = await db.Laboratorists.FindAsync(id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> EditLaboratorist(Laboratorists profile, HttpPostedFileBase picture, int? id)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("EditLaboratorist", "UserManager");
            }
        ...
        public async Task<ActionResult> MyProfile()
        {
            ...
            var profile = await db.Laboratorists.Include(x => x.User).FirstOrDefaultAsync(x => x.UserId == userinfo);
